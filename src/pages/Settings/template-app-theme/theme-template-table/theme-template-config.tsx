...
import { deleteThemeTemplateById } from "../../../../services/request";
const { Text } = Typography;

const filterData = (data: ThemeData[], byParam: string) => (formatter: any) => {
...
export const createColumnContent = (
  ...
  value: string,
  handleSearch: any,
  setData: Dispatch<SetStateAction<ThemeData[]>>,
  setDefaultData: Dispatch<SetStateAction<ThemeData[]>>
): ColumnsType<ThemeData> => {
  const handleDelete = async (recordId: any, text: any) => {
    try {
      await deleteThemeTemplateById(recordId);
      ...
      render: (text, record) => (
        <div className="flex">
          <Link
            ...
            to={`/settings/template-app-theme/edit/${record?.id}`}
          >
            {text}
          </Link>
          <div
